				...
				<div class="widget-content">
					...
					<form action="<?php echo $this->url->getUrl('administrator/Pages', 'delete'); ?>" method="post">
						<table class="table">
							...
							<tr>
								<td class="select">
									<?php if ($method['editable']) { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $method['controller'].'-'.$method['main_method'].'-'.$method['sub_method']; ?>" /></td>
									<?php } ?>
								...
								<td>
									<a href="<?php echo $this->url->getUrl('administrator/Pages', 'edit', [$method['controller'], $method['main_method'], $method['sub_method']]); ?>" class="btn btn-primary"><i class="fa fa-edit" title="<?php echo $text_edit; ?>"></i></a>
								</td>
							</tr>
							<?php } ?>
						</table>
						<button type="submit" class="btn btn-primary" name="form-page-list-submit" onclick="return deleteSelected();"><?php echo $text_delete_selected; ?></button>
					</form>
					<div class="pagination">
						<?php echo $pagination->getPageLinks(); ?>
					</div>
